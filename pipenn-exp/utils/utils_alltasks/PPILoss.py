...
class LossParams:
    ...
    ## kiki change: CROSS_ENTROPY > None
    LOSS_FUN = None
...
class PPILossCls(object):
    ...
    def maskPadTargetTensor(cls, y_true, y_pred):
        ...
        # print('y_true', y_true[0].shape, y_true[1].shape)
        ...
        if DatasetParams.MULTI_TASK_MODEL:
            ...
            # creating a dictionary with target_nr as key and true + pred as value 
            for i in range(len(y_pred)):
                target_dict[i] = list(cls.deletePadding(y_true[i], y_pred[i]))
                

            # values is a list of lists, each list contains a flat y_true, y_pred for a target 
            return list(target_dict.values())
        
        elif DatasetParams.USE_VAR_BATCH_INPUT:
    ...
    def accMetric(cls, y_true, y_pred):
        ...
        # print("11111111 %%%%%%%%%%%%%% np.argwhere y_true %%%%%%%%%%%", np.argwhere(np.isnan(y_true)))
        # print("22222222 %%%%%%%%%%%%%% np.argwhere y_pred %%%%%%%%%%%", np.argwhere(np.isnan(y_pred)))

        ## Kiki: make sure only theshold for binary classification 
        y_pred = y_pred >= LossParams.CLASS_THRESHOLD
    ...
    def logTrainingMetrics(cls, epoch, loss, valLoss, y_true, y_pred):
        ...
        elif DatasetParams.MULTI_TASK_MODEL:
            ## Kiki : now you get a list for each target with y_true and y_pred 
            ## y_1 = [masked_y_true, masked_y_pred] and y_2 = ...
            masked_target_list = cls.maskPadTargetTensor(y_true, y_pred)
        else:
        ...
        if DatasetParams.MULTI_TASK_MODEL:
            label_columns = DatasetParams.OUTPUT_COLUMNS

            # Make a dict with accuracy for all binary classifications
            for i in range(len(LossParams.BINARY_MASK)):
                if LossParams.BINARY_MASK[i] == 1:
                    y_true, y_pred = masked_target_list[i]
                    valAcc = cls.accMetric(y_true, y_pred) 
                    valAuc = cls.aucMetric(y_true, y_pred)

                    # only calculations for ppi 
                    specScore = cls.confMetric(y_true, y_pred, testingLabel=None)
                    ...
                    logger.info("Training Metrics {}, Epoch:{}  ValAcc: {} specScore: {:.2%} presScore: {:.2%} recallScore: {:.2%} F1Score: {:.2%} MCC: {:.2%} AUC: {:.2%} AP: {:.2%}".format(label_columns[i], epoch,
                        valAcc, specScore, presScore, recallScore, f1Score, mccScore, valAuc, valAp))
                    
                else:
                    # calculate Pearson for regression tasks
                    y_true, y_pred = masked_target_list[i]

                    # Calculate Pearson correlation coefficient for regression 
                    pearson_corr, p = pearsonr(y_true, y_pred)

                    logger.info("Training Metrics {},  pcc: {:.2%}, pvalue: {:.2%}".format(label_columns[i], pearson_corr, p))

            # returning y_true, y_pred for ppi 
            ppiAuc = cls.aucMetric(masked_target_list[0][0], masked_target_list[0][1])
        ...
        if DatasetParams.MULTI_TASK_MODEL:
            ...
            for i in range(len(LossParams.BINARY_MASK)):
                if LossParams.BINARY_MASK[i] == 1:
                    ...
                    logger.info("Testing Metrics {},  ValAcc: {} specScore: {:.2%} presScore: {:.2%} recallScore: {:.2%} F1Score: {:.2%} MCC: {:.2%} AUC: {:.2%} AP: {:.2%}".format(label_columns[i],
                        valAcc, specScore, presScore, recallScore, f1Score, mccScore, valAuc, valAp))
                    ...
                    logger.info("Testing Metrics {},  pcc: {:.2%}, pvalue: {:.2%}".format(label_columns[i], pearson_corr, p))


            # returning y_true, y_pred for ppi 
            return masked_target_list[0][0], masked_target_list[0][1]
    ...
    @classmethod
    def categoricalCrossEntropyLoss(cls, y_true, y_pred):
        # Compute categorical cross-entropy loss
        loss = y_true * -K.log(y_pred + K.epsilon())
        
        # Calculate mean loss
        cross_entropy = K.mean(loss)
    ...
    @classmethod
    def MTLoss_bce(cls, y_true, y_pred):

        print('mtloss_bce', y_true.shape, y_pred.shape)
        if LossParams.LOSS_FUN not in [LossParams.YOLO, LossParams.ATT, LossParams.MC_RESNET]:  
            if LossParams.USE_DELETE_PAD:
                y_true, y_pred = cls.deletePadding(y_true, y_pred)
    ...
    @classmethod
    def MTLoss_mse(cls, y_true, y_pred):

        print('mtloss_mse', y_true.shape, y_pred.shape)
    ...
    @classmethod
    def MTLoss_cross_entropy(cls, y_true, y_pred):

        print('mtloss_cross_entropy', y_true.shape, y_pred.shape)
        ...
        loss = cls.categoricalCrossEntropyLoss(y_true, y_pred) 
