...
def download_partial_csv(file_url, save_path, num_samples=100):
    try:
        df = pd.read_csv(file_url, nrows=num_samples)

        df.to_csv(save_path, index=False)

        print("First samples downloaded  succesfully.")
...
if __name__ == "__main__":
    ...
    save_location = ROOT_DIR + "epitope_training_partial.csv"
    AlgParams.initAlgParams()
    if AlgParams.USE_EXPLAIN:
        performExplanation()
        sys.exit(0)
    if not AlgParams.ONLY_TEST:
        download_partial_csv(EPITOPE_TRAINING_FILE, save_location)
       # performTraining()
    performTesting()
